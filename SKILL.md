...
description: Generate videos through a user-configured New API gateway using Seedance models only. Supports the documented A-series /v1/videos contract, legacy Seedance 2.0 -0826 routing, references, polling, and MP4 download.
metadata:
...
Use the bundled zero-dependency CLI for Seedance video generation. It reads `DRAMA_BASE_URL`, `DRAMA_API_KEY`, `DRAMA_MODEL`, and `DRAMA_ENDPOINT` from `.env` in the skill directory or the process environment. Non-Seedance models are filtered from discovery and rejected before submission.
...
- With `DRAMA_ENDPOINT=auto`, the CLI selects the route from the model id and rejects an explicitly mismatched route.
- The `models` command prints only model ids beginning with `seedance`; use ids returned by `GET /v1/models` for the configured token.
...
- Legacy `*-0826`: 4-15 seconds; fast supports up to 720p and does not accept video references.
